...
- Embed `example-fixtures` and `demo-skill` so `openeval demo --agent mock` works outside a checkout while explicit paths and aliases retain precedence.
...
- The shipped `example-fixtures` scenario now provides four independent standard-library Python maintenance tasks with focused scenario-owned graders whose parent assertions are isolated from workspace imports, plus a one-task credential-free mock subset.
